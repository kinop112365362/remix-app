...
import { getVariableSetter } from "@/pages/mo/setter/variableSetters"
...
export const replaceVariables = async (text: string, variables: Record<string, any>) => {
  let result = text
  for (const [key, value] of Object.entries(variables)) {
    const variableName = key === "input" || key === "MO_AI_STUDIO_USER_SYSTEM" ? key : value.name
    const regex = new RegExp(`{{${variableName}}}`, "g")

    if (value === undefined || value === null || (typeof value === "string" && value.trim() === "")) {
      message.error(`Variable ${variableName} is empty or does not exist. Key: ${key}, Value: ${value}`)
    }

    try {
      let parsedValue
      if (key === "input" || key === "MO_AI_STUDIO_USER_SYSTEM") {
        parsedValue = await getVariableSetter(value.setter).parse(value.value)
      } else {
        parsedValue = await getVariableSetter(value.setter).parse(value.value)
      }
      ...
      if (
        ...
        console.error(`Parsed value for ${variableName} is empty. Key: ${key}, Original Value: ${value.value}`)
      }
      //console.log(`%c[Variable Replacement] ${variableName}: ${parsedValue}`, "color: #4CAF50; font-weight: bold;")
      result = result.replace(regex, parsedValue)
    } catch (error) {
      console.error(`Error parsing variable ${variableName}. Key: ${key}, Value:`, value)
      throw error
    }
  }
  //console.log(`%c[Final Result] ${result}`, "color: #2196F3; font-weight: bold;")
  //console.log(`%c[Original Text] ${text}`, "color: #FF9800; font-weight: bold;")
  //console.log(`%c[Variables] ${JSON.stringify(variables, null, 2)}`, "color: #9C27B0; font-weight: bold;")
  return result
}

export const getAppId = () => sessionStorage.getItem("x-app-id") || new URL(location.href).searchParams.get("appId")
