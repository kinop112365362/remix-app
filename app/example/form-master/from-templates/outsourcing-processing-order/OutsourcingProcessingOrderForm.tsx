...
interface OutsourcingProcessingOrderFormProps {
  formId: string
  templateId: string
}

const OutsourcingProcessingOrderForm: React.FC<OutsourcingProcessingOrderFormProps> = ({ formId, templateId }) => {
  const { getFormById, addForm, updateFormStatus } = useFormMetadata()
    ...
    try {
      ...
      if (formData) {
        ...
        setForm({
          ...
          templateId: "outsourcingProcessingOrder",
          status: "draft",
          data: {
  ...
  const handleStateTransition = async () => {
    ...
    if (nextState) {
      try {
        ...
        setForm((prev: any) => ({ ...prev, status: nextState }))
        fetchForm()
      } catch (error) {
...
export default OutsourcingProcessingOrderForm
