...
interface ShataTesterProcessingReportProps {
  reportId: string
  data: any
}
...
const ShataTesterProcessingReport: React.FC<ShataTesterProcessingReportProps> = ({ reportId, data }) => {
  ...
  const processSupplierData = (chartData: ChartDataItem[]): { value: number; name: string }[] => {
    ...
    chartData.forEach((item) => {
      ...
      supplierLocations.push({
        ...
        orderCount: supplierCounts[supplierName]
      })
    })
  ...
  // 处理物料统计数据的函数
  const processMaterialStats = (chartData: ChartDataItem[]): MaterialStats[] => {
    const materialData: { [key: string]: MaterialStats } = {}

    chartData.forEach((item) => {
      item.data.items.forEach((materialItem) => {
        if (!materialData[materialItem.物料名称]) {
          materialData[materialItem.物料名称] = { name: materialItem.物料名称, totalQuantity: 0, totalAmount: 0 }
        }
        materialData[materialItem.物料名称].totalQuantity += materialItem.数量
        materialData[materialItem.物料名称].totalAmount += materialItem.总价
      })
    })
  ...
  useEffect(() => {
    async function getData() {
      ...
      // 处理物料统计数据
      const materialStatsData = processMaterialStats(chartData)
      setMaterialStats(materialStatsData)
  ...
  const renderMaterialStatistics = () => (
    <Card className='mb-4'>
      <CardHeader>
        <h3 className='text-lg font-semibold'>物料统计</h3>
      </CardHeader>
      <CardBody>
        <div className='space-y-4'>
          {materialStats.map((material) => (
            <div key={material.name} className='flex justify-between items-center'>
              ...
              <div>
                <span className='mr-4'>数量: {material.totalQuantity}</span>
                <span>金额: ¥{material.totalAmount.toFixed(2)}</span>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div>
      <h2 className='text-black'>沙塔测试委外加工报表 (ID: {reportId})</h2>
      ...
      <MapComponent data={supplierLocations}></MapComponent>
    </div>
  )
}

export default ShataTesterProcessingReport